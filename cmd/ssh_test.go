...
func TestCmdSSHListE2E(t *testing.T) {
	...
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if !strings.Contains(r.URL.Path, "organization/org-123/ssh") {
	...
	out, err := executeCommand("ssh", "list", "--organization-id", "org-123")
	if err != nil {
...
func TestCmdSSHGetE2E(t *testing.T) {
	...
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if !strings.Contains(r.URL.Path, "organization/org-123/ssh/ssh-456") {
			t.Errorf("unexpected path: %s", r.URL.Path)
		}
	...
	out, err := executeCommand("ssh", "get", "--organization-id", "org-123", "--id", "ssh-456")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestCmdSSHDeleteE2E(t *testing.T) {
	...
	out, err := executeCommand("ssh", "delete", "--organization-id", "org-123", "--id", "ssh-del")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
