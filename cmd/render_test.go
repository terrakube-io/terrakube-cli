...
// splitInterface panics on nil Attributes — no nil check before reflect.Indirect.
func TestSplitInterface_NilAttributes(t *testing.T) {
	...
	defer func() {
		r := recover()
		if r == nil {
			t.Fatal("expected panic when Attributes is nil, but did not panic")
		}
		// Panic is expected: reflect.Indirect on nil Attributes causes a
		// nil pointer dereference because splitInterface calls
		// reflect.Indirect(reflect.ValueOf(d.FieldByName("Attributes").Interface()))
		// without checking for nil first.
		t.Logf("confirmed panic on nil Attributes: %v", r)
	}()
